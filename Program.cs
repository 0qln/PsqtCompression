...
static class Program
{
    ...
    static double _lastAvgDelta = 0;
    static double _lastStdDeviation = 0;
    static void PrintArray<T>(T[] array, T[] expected)
    {
        for (int i = 0; i < array.Length; i++)
        {
            var delta = Math.Abs(Math.Abs(Convert.ToDouble(array[i])) - Math.Abs(Convert.ToDouble(expected[i])));
            Console.WriteLine(
    ...
    {
        for (int compressionRate = 0; compressionRate < 400; compressionRate += 10)
        {
            Console.WriteLine($"COMPRESSION; {compressionRate}");

            var f_hat = DFT.Forward(mg_pawn_table
                .Select(x => (int)x)
                .ToArray());

            var f = DFT.Inverse(f_hat
                .Select(x => x)
                .ToArray());

            Complex[] f_hat_compressed = DFT.Compress(f_hat, compressionRate);
            var f_compressed = DFT.Inverse(f_hat_compressed);

            Console.WriteLine();
            for (int i = 0; i < f_hat.Length; i++)
            {
                var delta = (f_hat[i] -  f_hat_compressed[i]);
                Console.WriteLine(
                $"Value: {f_hat[i].ToString()?.PadRight(50)}" +
                $"Expected: {f_hat_compressed[i].ToString()?.PadRight(50)}" +
                $"Delta: {delta}");
            }
            Console.WriteLine();

            //PrintArray(f, mg_pawn_table.Select(Convert.ToDouble).ToArray());
            //Console.WriteLine($"Avarage Delta: {_lastAvgDelta}");

            Console.WriteLine();
            PrintArray(f_compressed, mg_pawn_table.Select(Convert.ToDouble).ToArray());
            Console.WriteLine($"Avarage Delta: {_lastAvgDelta}");
        }
